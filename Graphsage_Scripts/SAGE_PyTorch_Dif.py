...
def load_node_csv(path, index_col, **kwargs):
    ...
    mapping = {index: i for i, index in enumerate(np.unique(df[['source', 'target']].values))}
    
    x = np.identity(len(np.unique(df[['source', 'target']].values)))
...
for cell in cells:
    print(cell)
    node_x, node_mapping = load_node_csv("/mnt/dv/wid/projects3/Roy-enhancer-promoter/Zhiwei_Work/RoadMap_Networks/Data/Roadmap_Networks/edgelist/{}_no_weight.edgelist".format(cell), index_col='source')
    data = Data()
    data.x = node_x
    print(data)
    edge_index = load_edge_csv_noweight("/mnt/dv/wid/projects3/Roy-enhancer-promoter/Zhiwei_Work/RoadMap_Networks/Data/Roadmap_Networks/edgelist/{}_no_weight.edgelist".format(cell), x_mapping = node_mapping)
    print(edge_index)
    ...
    with open("/mnt/dv/wid/projects3/Roy-enhancer-promoter/Zhiwei_Work/RoadMap_Networks/Results/embeddings/graphsage_256d/{}_features.emb".format(cell), "w") as f:
