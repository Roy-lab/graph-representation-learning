...
END{
	...
	for (d=1; d<=2; d++)
	{
		...
		for (z=1; z<=NumZ[3-d]; z++) 
			if ((ZEx[3-d,z] == "" || (ZEx[3-d,z] ~ /^Text/)) && ZN[3-d,z] != "")
	...
	for (d=1; d<=2; d++)
		for (z=1; z<=NumZ[d]; z++)
			if (ZEx[d,z] == "" || (ZEx[d,z] ~ /^Text/ && !H("SQ", ZEx[d,z], 4)))
				print text(d, ZN[d,z], ZP[3-d,1] - 4, (ZP[d,z] + ZP[d,z+1])/2, "", 1)
			else if (ZEx[d,z] ~ /^Text/ && H("S", ZEx[d,z], 4))
				print text(d, ZN[d,z], ZP[3-d,1] - 11, ZP[d,z] + 3, 1, 1)
			else if (ZEx[d,z] ~ /^Text/ && H("Q", ZEx[d,z], 4))
			...
			if ((ZEx[1,r] == "" || ZEx[1,r] ~ /^Text/) && (ZEx[2,c] == "" || ZEx[2,c]  ~ /^Text/))
			{
				...
				for (v=1; v<=nv; v++)
					print rect((VA[v] == "") ? D : VA[v], ZP[2,c], ZP[1,r], ZP[2,c+1], ZP[1,r+1], 
						...
						(((ZEx[1,r] ~ /^Text/ && H("R", ZEx[1,r], 4)) || (ZEx[2,c] ~ /^Text/ && H("R", ZEx[2,c], 4))) ? "R" : (((ZEx[1,r] ~ /^Text/ && H("C", ZEx[1,r], 4)) || (ZEx[2,c] ~ /^Text/ && H("C", ZEx[2,c], 4)) || !(ZEx[1,r] ~ /^Text/ || ZEx[2,c] ~ /^Text/)) ? "C" : "L")) ((ZEx[1,r]  ~ /^Text/) ? "S" : "F"))
			}
...
function poly (dim,  xs, ys, cl, str,  Xa, Ya, n, i, L)
{
	...
	return X(cl str, "<polygon points=" Q(substr(L,2)) " fill=" Q(X(cl,cl,"none")) X(str, " stroke=" Q(str) " stroke-width=" Q(StrokeW)) "/>")
}
