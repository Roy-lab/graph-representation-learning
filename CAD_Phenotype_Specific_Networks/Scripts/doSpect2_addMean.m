...
filename = {'breast_variant_human_mammary_epithelial_cells_vhmec.txt', 'cd14_primary_cells.txt', 'cd19.txt', 'cd34_primary_cells.txt', 'cd3_primary_cells.txt', 'cd4_primary_cells.txt', ...
    'cd56_primary_cells.txt', 'cd8_primary_cells.txt', 'fetal_adrenal_gland.txt', 'fetal_brain.txt', 'fetal_heart.txt', 'fetal_intestine_large.txt', ...
    ...
    'fibroblasts_fetal_skin_upper_back.txt', 'gastric_mucosa.txt', 'heart.txt', 'h1_bmp4_derived_mesendoderm_cultured_cells.txt', 'h1_bmp4_derived_trophoblast_cultured_cells.txt', 'h1_cells.txt', ...
...
for y = 1:length(filename)
    ...
    for k=10:10:100
        ...
        cids=cidx;
        writetable(table(cids), sprintf('/mnt/dv/wid/projects3/Roy-enhancer-promoter/Zhiwei_Work/RoadMap_Networks_Cad/Results/spect_kmeans/%s/%d_cluster_spect_kmeans.txt', cell, k),'delimiter',',','writerownames',false,'writevariablenames',false)
    end
end
