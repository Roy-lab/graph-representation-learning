...
for i = 1:length(cells)
    mat = importdata(sprintf('/mnt/dv/wid/projects3/Roy-enhancer-promoter/Zhiwei_Work/RoadMap_Networks_Org/Results/embeddings/%s/%s_features.emb',folder,cells{i}));
    disp(folder);
    disp(size(mat));
    cell = cells{i};
    disp(cells{i});
    ...
    for k = 10:10:100
        ...
        saveas(f, sprintf('/mnt/dv/wid/projects3/Roy-enhancer-promoter/Zhiwei_Work/RoadMap_Networks_Org/Results/node_feature_kmeans/%s/%s/%d_cluster_kmeans.pdf', folder, cell, k));

        cids = cidx;
        result = table(cids);
        writetable(result, sprintf('/mnt/dv/wid/projects3/Roy-enhancer-promoter/Zhiwei_Work/RoadMap_Networks_Org/Results/node_feature_kmeans/%s/%s/%d_cluster_kmeans.txt', folder, cell, k),'delimiter',',','writerownames',false,'writevariablenames',false)
    end
end
