...
filename = ['breast_variant_human_mammary_epithelial_cells_vhmec.txt', 'cd19.txt', 'cd34_primary_cells.txt', 'cd3_primary_cells.txt', \
    'cd4_primary_cells.txt', 'cd56_primary_cells.txt', 'cd8_primary_cells.txt', 'fetal_adrenal_gland.txt', 'fetal_heart.txt', 'fetal_intestine_large.txt', \
    ...
    'fibroblasts_fetal_skin_upper_back.txt', 'gastric_mucosa.txt', 'h1_bmp4_derived_mesendoderm_cultured_cells.txt', 'h1_bmp4_derived_trophoblast_cultured_cells.txt', \
    ...
    'psoas_muscle.txt', 'small_bowel_mucosa.txt', 'testes.txt', 'cd14_primary_cells.txt', 'fetal_brain.txt', 'heart.txt']

for file in filename:

    data = pd.read_csv('..//Data//adj_matrix//' + file, sep="\t", header = None)
    print(data.shape)
    ...
    #edge_list.to_csv("..//Data//Roadmap_Networks//edgelist//" + file[:-4] + ".txt", index = False, header = False, sep=' ')
    np.savetxt("..//Data//edgelist//" + file[:-4] + ".edgelist", edge_list[['From', 'To']], delimiter='\t',fmt ="%i")