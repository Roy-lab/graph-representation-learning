...
for (cell in cells){
  ...
  print(cell)
  for (k in x) {
    netfile <- sprintf("Data//edgelist//%s.edgelist", cell)
    clstfile <- sprintf("Results//spect_kmeans//%s//%d_cluster_spect_kmeans_names_dsd.txt", cell, k)
    #clstfile <- sprintf("Results//node_feature_kmeans//%s//%s//%d_cluster_kmeans_names.txt", args, cell, k)
    library('igraph')
